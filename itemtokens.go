...
func (l LBD) RetrieveTheStatusOfMultipleNonFungibleTokenIcons(contractId, requestId string) (*NonFungibleTokenResponse, error) {
	...
	ret := new(NonFungibleTokenResponse)
	return ret, json.Unmarshal(resp.ResponseData, ret)
}
...
func (r MintMultipleNonFungibleRequest) Encode() string {
	...
	mintList := fmt.Sprintf("%smintList.name=%s&mintList.tokenType=%s",
		emptyMintListData(metas, "mintList.meta"),
		strings.Join(names, ","),
...
type UpdateMultipleToken struct {
	RequestId string `json:"request-id"`
}

type UpdateMultipleTokenRequest struct {
	*Request
	UpdateList []*UpdateList `json:"updateList"`
}

func (r UpdateMultipleTokenRequest) Encode() string {
	base := r.Request.Encode()
	types := make([]string, len(r.UpdateList))
	...
	updateList := fmt.Sprintf("updateList.tokenType=%s", strings.Join(types, ","))
	fmt.Println(fmt.Sprintf("%s?%s", base, updateList))

	return fmt.Sprintf("%s?%s", base, updateList)
}

func (l *LBD) UpdateMultipleFungibleTokenIcons(contactId string, updateList []*UpdateList) (*UpdateMultipleToken, error) {
	path := fmt.Sprintf("/v1/item-tokens/%s/fungibles/icon", contactId)

	r := UpdateMultipleTokenRequest{
		Request:    NewPutRequest(path),
		UpdateList: updateList,
	}
...
func (l *LBD) AttachNonFungibleAnother(contractId, tokenType, tokenIndex, tokenId, tokenHolder string) (*Transaction, error) {
	path := fmt.Sprintf("/v1/item-tokens/%s/non-fungibles/%s/%s/parent", contractId, tokenType, tokenIndex)
	...
	if l.IsAddress(tokenHolder) {
		r.TokenHolderAddress = tokenHolder
	} else {
		r.TokenHolderUserId = tokenHolder
	}
...
func (l *LBD) DetachNonFungibleParent(contractId, tokenType, tokenIndex, tokenHolder string) (*Transaction, error) {
	path := fmt.Sprintf("/v1/item-tokens/%s/non-fungibles/%s/%s/parent", contractId, tokenType, tokenIndex)
...
type MintMultipleRecipientList struct {
	TokenType string `json:"tokenType"`
	Name      string `json:"name"`
	Meta      string `json:"meta,omitempty"`
	ToUserId  string `json:"toUserId,omitempty"`
...
type MintMultipleNonFungibleRecipientsRequest struct {
	*Request
	OwnerAddress string                       `json:"ownerAddress"`
	OwnerSecret  string                       `json:"ownerSecret"`
	MintList     []*MintMultipleRecipientList `json:"mintList"`
}
...
func (r MintMultipleNonFungibleRecipientsRequest) Encode() string {
	...
	mintList := fmt.Sprintf("%smintList.name=%s&%s%smintList.tokenType=%s",
		emptyMintListData(metas, "mintList.meta"),
		strings.Join(names, ","),
		emptyMintListData(toAddresses, "mintList.toAddress"),
		emptyMintListData(toUserIds, "mintList.toUserId"),
		strings.Join(tokenTypes, ","),
	)

	return fmt.Sprintf("%s?%s&ownerAddress=%s&ownerSecret=%s", base, mintList, r.OwnerAddress, r.OwnerSecret)
}

func (l *LBD) MintMultipleNonFungibleRecipients(contractId string, mintList []*MintMultipleRecipientList) (*Transaction, error) {
...
func emptyMintListData(data []string, print string) string {
	isEmpty := false
	for _, m := range data {
		if m != "" {
			isEmpty = true
	...
	var path string
	if isEmpty {
		path = fmt.Sprint(print+"=", strings.Join(data, ",")+"&")
	} else {
		path = ""
	}
	return path
